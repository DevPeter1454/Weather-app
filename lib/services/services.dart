      ...
      retrieved() {
        for (var i = 0; i < retrievedData['list'].length; i++) {
          list.add(OpenWeather(
              cloud: retrievedData['list'][i]['weather'][0]['main'],
              description: retrievedData['list'][i]['weather'][0]
                  ['description'],
              icon: retrievedData['list'][i]['weather'][0]['icon'],
              temperature: retrievedData['list'][i]['main']['temp'],
              date: DateTime.fromMillisecondsSinceEpoch(
                  retrievedData['list'][i]['dt'] * 1000)));
        }
        infos = {'list': list, 'others': retrievedData['city']};
      }

      // data.body.isEmpty ? null :  retrieved();
      retrieved();
      return infos;
    } else {
      return null;
